...
/**
 * Android implementation that reads Claude Code usage data directly from local files.
 * Useful when Claude Code runs on the same device (e.g. via Termux) or when
 * .claude/ files are synced to the device.
 *
 * If no custom path is set, it tries common locations:
 * - Termux home: /data/data/com.termux/files/home/.claude/
 * - Shared storage: /storage/emulated/0/.claude/
 * - App-internal fallback
 */
class AndroidLocalFileUsageDataSource(
    customPath: String = ""
) : UsageDataSource {

    private val claudeHome: File = if (customPath.isNotBlank()) {
        File(customPath)
    } else {
        detectClaudeHome()
    }
    ...
    override suspend fun checkConnection(): Boolean = withContext(Dispatchers.IO) {
        claudeHome.exists() && claudeHome.isDirectory
    }
            ...
            try {
                if (!claudeHome.exists()) {
                    return@withContext Result.failure(
                        Exception("Claude data directory not found: ${claudeHome.absolutePath}")
                    )
                }
    ...
    private fun detectClaudeHome(): File {
        // Try Termux home first
        val termuxHome = File("/data/data/com.termux/files/home/.claude")
        if (termuxHome.exists()) return termuxHome

        // Try shared storage
        val sharedStorage = File("/storage/emulated/0/.claude")
        if (sharedStorage.exists()) return sharedStorage

        // Default to Termux path (most likely use case)
        return termuxHome
    }
